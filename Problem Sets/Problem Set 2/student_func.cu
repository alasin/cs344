                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  const int index = y_index * numCols + x_index;
  ...
  for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r)
  {
    ...
    {
      int image_r = min(max(y_index + filter_r, 0), static_cast<int>(numRows - 1));
      int image_c = min(max(x_index + filter_c, 0), static_cast<int>(numCols - 1));
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  checkCudaErrors(cudaMemcpy((void *)d_filter,  (void *)h_filter, filterSize, cudaMemcpyHostToDevice));
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize(numCols/threadsPerBlock + 1, numRows/threadsPerBlock + 1);
